...
then
  ...
  sudo nmap $target -p- -T4 -Pn -sC -sV -oA ~/HackSpace/$name/recon/init_scan.txt | grep "open" | cut -d "/" -f 1 > ~/HackSpace/$name/recon/openports.txt
  ...
  echo "[*] Nmap complete. It's fuzz o' clock"
